...
class MediaEloquent extends BaseEloquent {
    ...
    public function getVideoInfoByLink($link) {
        ...
        if (strpos($link, 'youtube.com')) {
            parse_str(parse_url(trim($link), PHP_URL_QUERY), $array_values);
            $video_id = $array_values['v'];
            $result['video_id'] = $video_id;
            $result['thumbnail'] = 'https://img.youtube.com/vi/'.$video_id.'/0.jpg';
            
            $client = new Client();
            $res = $client->request('GET', $link);
            $resContent = $res->getBody()->getContents();
            
            $crawler = new Crawler($resContent);
            $result['name'] = utf8_decode(trim($crawler->filter('#eow-title')->text()));
            $result['description'] = utf8_decode(trim($crawler->filter('#eow-description')->text()));
        }
        return $result;
    }
